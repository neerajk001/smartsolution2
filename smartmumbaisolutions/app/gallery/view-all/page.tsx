...
export default function ViewAllPhotosPage() {
    ...
    async function loadAllImages() {
      try {
        ...
        if (response.success && response.events.length > 0) {
          // Combine all images from all events
          const images = response.events.flatMap(event => getSortedImages(event));
          setAllImages(images);
          
          // Use first event's details for the page header
          setEventTitle(response.events[0].title);
          setEventDescription(response.events[0].description);
        } else {
          // Fallback to static images if API fails
          const fallbackImages: GalleryImage[] = [
            {
              id: 1,
              imageUrl: "/events/event (2).jpeg",
              altText: "Christmas Festival Celebration 1",
              displayOrder: 0,
              isFeatured: true,
            },
            {
              id: 2,
              imageUrl: "/events/event (3).jpeg",
              altText: "Christmas Festival Celebration 2",
              displayOrder: 1,
              isFeatured: false,
            },
            {
              id: 3,
              imageUrl: "/events/event (4).jpeg",
              altText: "Christmas Festival Celebration 3",
              displayOrder: 2,
              isFeatured: false,
            },
            {
              id: 4,
              imageUrl: "/events/event (5).jpeg",
              altText: "Christmas Festival Celebration 4",
              displayOrder: 3,
              isFeatured: false,
            },
            {
              id: 5,
              imageUrl: "/events/event (6).jpeg",
              altText: "Christmas Festival Celebration 5",
              displayOrder: 4,
              isFeatured: false,
            },
            {
              id: 6,
              imageUrl: "/events/event (7).jpeg",
              altText: "Christmas Festival Celebration 6",
              displayOrder: 5,
              isFeatured: false,
            },
          ];
          ...
          setEventTitle("Christmas Festival Gallery");
          setEventDescription("All photos from our Christmas Festival celebration");
          
          if (!response.success) {
            setError(response.error || 'Using fallback images');
          }
        }
      } catch (err) {
